...
namespace HoloToolkit.Unity.SpatialMapping
{
    ...
    public static class MeshSaver
    {
        ...
        //cyshen
        public static void saveWaveFront(string fileName, IEnumerable<MeshFilter> meshFilters)
        {
            // cyshen 042018
            String folderName = MeshFolderName;
            Stream s = OpenFileForWrite(folderName, fileName + ".obj");
            using (StreamWriter stream = new StreamWriter(s))
            {
                stream.Write(SerializeMeshFilters(meshFilters));
            }
        }    
